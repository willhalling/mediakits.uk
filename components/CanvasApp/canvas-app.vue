...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            changeBackground(event) {
                ...
                if (files) {
                    ...
                    Array.from(files).forEach(file => {
                        ...
                        imageUploadPromises.push(new Promise( (resolve, reject) => {
                                        ...
                                        return EXIF.getData(file, function () {
                                            return findOrientation(file, (error, orientation) => {
                                                if (!error) {
                                                    ...
                                                    resolve({
                                                        date: exifDateTime && parseDate(exifDateTime).toString(),
                                ...
                                getExif().then(exifImage => {
                                    ...
                                    getImageSpec(rotate).then(imageWithSpecs => {
                                        image = {
                                            id: uuidv4(),
                                            ...imageWithSpecs,
                                            age: exifImage.date ? this.updateAge(this.rawBirthday, exifImage.date) : null,
                                            date: exifImage.date,
                                            orientation: {
                                                rotate: rotate
                                            },
                                            // Error because photo was taken after baby's birthday.
                                            error: new Date(exifImage.date) < new Date(this.rawBirthday)
                                        }
            ...
            updateAge(date, photoDate = new Date()) {
                let age = getAge(date, photoDate)
                age = {
                    ...age,
                    ...this.getAgeNumber(age),
                    text: ageText(age),
                    path: this.numberPath(age)
                }
                return age
            },
            addSvg(svg) {
            ...
            /*
            deleteObject(type) {
              this.$refs.canvasContainer.deleteActiveObject()
